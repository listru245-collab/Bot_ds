...
client.once('ready', async () => {
  ...
  const data = [
    ...
    {
      name: 'copy',
      description: 'Скопировать эмодзи',
      options: [{
        name: 'emojis',
        type: 3, // STRING
        description: 'Эмодзи через пробел',
        required: true
      }]
    },
    {
      name: 'setmsg',
      description: 'Создать embed с кнопками'
    },
    {
      name: 'setowner',
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'setroles') {
      ...
      // РЕШЕНИЕ: ОДИН Embed, но картинки вставляем через .setImage() в ОТДЕЛЬНЫЕ невидимые Эмбеды
      // Discord склеивает эмбеды без левых цветных полосок в ОДНУ цельную рамку!
      
      const invisibleColor = 0x2b2d31;
      ...
      const embed2 = new EmbedBuilder()
        .setDescription("Получи интересующие тебя **роли уведомлений**, которые ты бы не хотел пропускать. Для этого **выбери роли** из списка ниже.\n\n────────────────────────────────────────────")
        .setColor(invisibleColor);
      ...
      const embed4 = new EmbedBuilder()
        .setDescription("Получи интересующие тебя **роли увлечений**, чтобы получать уведомления на интересующие тебя **ивенты**. Для этого **выбери роли** из списка ниже.\n\n────────────────────────────────────────────")
        .setColor(invisibleColor);
      ...
      const embed6 = new EmbedBuilder()
        .setDescription("Выбери роль **пола**, чтобы участники сервера могли лучше понимать, как к тебе обращаться.")
        .setColor(invisibleColor);
      ...
      // Отправляем все 6 склеенных эмбедов в одном сообщении
    ...
    if (commandName === 'copy') {
      ...
      for (const part of parts) {
        ...
        if (match) {
          ...
          try {
            ...
            const created = await interaction.guild.emojis.create({ attachment: buffer, name: name });
            results.push(`✅ ${created.toString()} \`:${name}:\``);
            success++;
          } catch (e) {
            results.push(`❌ \`${name}\` — ошибка загрузки (${e.message})`);
            failed++;
          }
        }
      }
      ...
      if (results.length === 0) {
        await interaction.editReply("Не найдено кастомных эмодзи в вашем запросе.");
        return;
      }

      const desc = `**Успешно:** ${success}\n**Не удалось:** ${failed}`;
      const logEmbed = makeLogEmbed("Копирование эмодзи", desc, 0x57F287, interaction.user);
      logEmbed.addFields({ name: "Результаты:", value: results.slice(0, 15).join("\n") });
      await interaction.editReply({ embeds: [logEmbed] });
      await sendLog(interaction.guild, logEmbed);
    }

    if (commandName === 'setmsg') {
      ...
      const imageInput = new TextInputBuilder()
        .setCustomId('image')
        .setLabel('Ссылка на картинку (необязательно)')
  ...
  // ОБРАБОТКА ПОДТВЕРЖДЕНИЯ МОДАЛОК
  if (interaction.isModalSubmit()) {
    if (interaction.customId === 'setmsg_modal') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(0x5865F2)
        .setFooter({ text: `Fail Bot — Создано администрацией` });

      if (image && image.startsWith("http")) {
        embed.setImage(image);
      }

      // Отправляем готовое сообщение в канал
      const sent = await interaction.channel.send({ embeds: [embed] });
      await interaction.reply({ content: `✅ Embed успешно отправлен! ID сообщения: \`${sent.id}\``, ephemeral: true });
    }
  }
